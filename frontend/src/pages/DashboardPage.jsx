...
function DashboardPage() {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="rounded-2xl border border-white/5 bg-white/5 p-6 shadow-card backdrop-blur-md">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-lg font-semibold">Top Dispositivos</h2>
          <span className="text-xs uppercase tracking-[0.2em] text-white/50">Ranking por consumo</span>
        </div>
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="text-white/60">
              <tr className="border-b border-white/10">
                ...
                <th className="py-2 font-medium">Franquia</th>
                <th className="py-2 font-medium">Consumo</th>
                <th className="py-2 font-medium">% Consumida</th>
              </tr>
            </thead>
            <tbody>
              {overview?.devices?.length ? (
                overview.devices
                  .slice()
                  .sort((a, b) => (b.usage?.totalMb || 0) - (a.usage?.totalMb || 0))
                  .map((device) => (
                    <tr key={device.id} className="border-b border-white/5 last:border-none">
                      <td className="py-3 font-medium text-white">{device.name}</td>
                      <td className="py-3 text-white/60">{device.simNumber || '—'}</td>
                      <td className="py-3 text-white/60">
                        {device.dataLimitMb ? formatMegabytes(device.dataLimitMb) : 'Sem limite'}
                      </td>
                      <td className="py-3 text-white">{formatMegabytes(device.usage?.totalMb || 0)}</td>
                      <td className="py-3 text-white/60">
                        {device.usage?.percentage ? `${device.usage.percentage}%` : '—'}
                      </td>
                    </tr>
                  ))
              ) : (
                <tr>
                  <td colSpan="5" className="py-6 text-center text-white/50">
                    Cadastre dispositivos para visualizar o ranking de consumo.
                  </td>
                </tr>
              )}
            </tbody>
